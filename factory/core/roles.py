...
async def _call_single_ai_provider(
    ...
    if provider == "cohere":
        from factory.integrations.cohere_provider import call_cohere
        return await call_cohere(prompt, contract)
    if provider == "fireworks":
        from factory.integrations.fireworks_provider import call_fireworks
        return await call_fireworks(prompt, contract)
    if provider == "nvidia_nim_glm":
        from factory.integrations.nvidia_nim_provider import call_nvidia_nim_glm
        return await call_nvidia_nim_glm(prompt, contract)
    if provider == "nvidia_nim_minimax":
        from factory.integrations.nvidia_nim_provider import call_nvidia_nim_minimax
        return await call_nvidia_nim_minimax(prompt, contract)
    if provider == "nvidia_nim_phi4":
        from factory.integrations.nvidia_nim_provider import call_nvidia_nim_phi4
        return await call_nvidia_nim_phi4(prompt, contract)
    if provider == "mock":
        return (f"[MOCK:{contract.role.value}] {prompt[:80]}", 0.0001)
