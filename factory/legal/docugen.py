...
async def _generate_single_document(
    ...
    # v5.8.16 Issue 69: supply actual values so the AI never needs
    # placeholder tokens. Old prompt said "include [EFFECTIVE_DATE]
    # and [COMPANY_NAME] placeholders" which then caused the quality
    # gate (check_no_placeholders) to reject every document.
    ...
    doc = await call_ai(
        ...
        prompt=(
            f"Write a complete, production-quality {doc_name}.\n"
            f"Structure:\n{structure[:4000]}\n"
            f"App Name: {app_name}\n"
            f"Effective Date: {_today}\n"
            f"Company / Operator: {app_name} (the operator of this application)\n"
            f"Business Model: {business_model}\n"
            f"Requirements:\n"
            f"  - Professional legal language, PDPL compliant, KSA jurisdiction\n"
            f"  - Bilingual Arabic + English where applicable\n"
            f"  - Write minimum 5 detailed sections with real legal substance\n"
            f"  - CRITICAL: Do NOT use bracket placeholders like [COMPANY_NAME],\n"
            f"    [EFFECTIVE_DATE], [INSERT HERE], [UNVERIFIED], or any [ALL_CAPS]\n"
            f"    token — substitute real values directly in the text.\n"
            f"  - If you do not know a specific value (e.g. physical address),\n"
            f"    write a realistic plausible value or omit the field entirely.\n"
            f"Return Markdown."
        ),
        state=state,
        action="write_code",
    ...
    # ── Post-process: fill any remaining placeholder tokens ──
    # Belt-and-suspenders: even if the AI slips a placeholder through,
    # replace it with real data before the quality gate sees it.
    import re as _re
    doc = _re.sub(r"\[EFFECTIVE_DATE\]", _today, doc)
    doc = _re.sub(r"\[COMPANY_NAME\]", app_name, doc)
    doc = _re.sub(r"\[APP_NAME\]", app_name, doc)
    doc = _re.sub(r"\[UNVERIFIED\]", "", doc)
    doc = _re.sub(r"\[INSERT[^\]]*\]", "", doc)
    doc = _re.sub(r"\[YOUR[^\]]*\]", "", doc)
    doc = _re.sub(r"\[DATE\]", _today, doc)
